#! /usr/bin/env python
#######################################################################################################
# Developed by: Sagnik Banerjee, Valeria Velasquez zapata

# Version 2 vvz
# changes in fusion read filtering, final report with gene name, update conda environment
...
def analyzeCommandLineArguments(options,logger_proxy,logging_mutex):
    ...
    options.record_time={}
    options.design_primers_for_transcript=options.output_directory+"/"+options.output_directory.split("/")[-1]+"_transcriptome_file_for_primer_design.fasta"
...
def alignReadsWithStarForTrimming(options,logger_proxy,logging_mutex):
    ...
    return"""
    #vvz adds print(options.star_genome_index)
    print("genome index: "+options.star_genome_index)
    if options.star_genome_index is None or options.star_genome_index=="":
        options.star_genome_index=options.temp_output_directory+"/star_index_with_vectors_with_transcriptome"
...
#    #if len(all_hamming_distances)==0 or len([x for x in all_hamming_distances if (x==min(all_hamming_distances))])>1: # change vvz to discard multiple hits of vector
...
def findVectorInReadPrefix(read_id,read_seq,vector,options,start,end):
    ...
    # Check for presence of vector in the 5 prime end of read_seq
    first_start=start
    min_hamming_distances=[10]
    """if "read13530682" in read_id:
        print(read_seq)"""
    while start<=end:
        """if "read13530682" in read_id:
            print(start,vector[-start:],read_seq[:start])"""
        seq_hamming_distance=hamming_distance(vector[-start:],read_seq[:start])
        if seq_hamming_distance<any(min_hamming_distances):
            min_hamming_distances=[seq_hamming_distance]
        if seq_hamming_distance in min_hamming_distances:
            min_hamming_distances.append(seq_hamming_distance)
        #all_hamming_distances.append(hamming_distance(vector[-start:],read_seq[:start]))
        start+=1
    """if "read13530682" in read_id:
        print(all_hamming_distances)"""
    #if len(all_hamming_distances)==0:
    if len(min_hamming_distances)!=1: # change vvz to discard multiple hits of vector
        return 10,10 # Sending a high value of hamming distance so that this read gets eliminated from being characterized as a fusion read
    return min(min_hamming_distances),min_hamming_distances.index(min(min_hamming_distances))+first_start
...
def findVectorInReadSuffix(read_id,read_seq,vector,options,start,end):
    ...
    if len(min_hamming_distances)!=1: # change vvz to discard multiple hits of vector
        return 10,10 # Sending a high value of hamming distance so that this read gets eliminated from being characterized as a fusion read 
    return min(min_hamming_distances),len(read_seq)-min_hamming_distances.index(min(min_hamming_distances))-first_start
